...
<!-- The docker image is based on [holy-build-box](https://github.com/phusion/holy-build-box) but modified by [rany2](https://github.com/rany2/) to also include golang and git. The reason for the need of Docker is so that we could be sure that the package will run on extremely old Debian and Ubuntu versions. Currently this package will work on Debian >= 7 and Ubuntu >= 10.10 thanks to the Docker image. -->
